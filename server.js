...
setInterval(async () => {
    ...
    try {
        ...
        const groupBlasts = campaigns.filter(c => c.type === 'group_blast' && c.status !== 'paused');

        for (const blast of groupBlasts) {
            const nextRun = blast.nextRun ? new Date(blast.nextRun) : new Date(0);
            ...
            if (now >= nextRun) {
                ...
                if (blast.frequency === 'hourly') {
                    ...
                    // Diário: Amanhã no mesmo horário
                ...
                await db.saveCampaign(blast);
                
                // Atualiza a lista no frontend
                const allC = await db.getAllCampaigns();
                io.to(blast.owner.toLowerCase()).emit('campaigns:list', allC.filter(c => c.owner === blast.owner));
            }
        }
    } catch (e) {
