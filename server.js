...
const app = express();
app.set('trust proxy', 1); // Essencial para o Rate Limit funcionar corretamente com IPs IPv6 e Proxys
const server = http.createServer(app);
// =================================================================================
// CONFIGURAÇÃO DO SOCKET.IO (MELHORADA PARA ESTABILIDADE)
// =================================================================================
const io = new Server(server, {
    maxHttpBufferSize: 20e6, // Aumentado para 20 MB para evitar queda em uploads grandes
    pingTimeout: 90000,       // 90 segundos (mais tolerante a instabilidades)
    pingInterval: 30000,      // 30 segundos (menos carga de pings)
...
// --- BANCO DE DADOS EM JSON PARA O IPTV (CORRIGE O BUG DE NÃO SALVAR) ---
const IPTV_FILE = path.join(BASE_DIR, 'iptv_data.json');
let iptvData = {};
if (fs.existsSync(IPTV_FILE)) {
    try { iptvData = JSON.parse(fs.readFileSync(IPTV_FILE, 'utf8')); } catch (e) { iptvData = {}; }
}

// --- FILA DE COBRANÇAS EM JSON ---
const COBRANCAS_FILE = path.join(BASE_DIR, 'cobrancas_queue.json');
let cobrancasQueue = {};
if (fs.existsSync(COBRANCAS_FILE)) {
    try { cobrancasQueue = JSON.parse(fs.readFileSync(COBRANCAS_FILE, 'utf8')); } catch (e) { cobrancasQueue = {}; }
}

// Processador de Disparos em Massa em Grupos (Recorrência)
setInterval(async () => {
    const now = new Date();
    try {
        // Busca campanhas diretamente do arquivo JSON para garantir sincronia com as rotas
        ...
        const groupBlasts = campaigns.filter(c => c.type === 'group_blast' && c.status !== 'paused' && c.status !== 'canceled');
        let needsSave = false;

        for (let blast of groupBlasts) {
            // Se não tem nextRun, ignora até que a rota de criação o gere ou use a data de criação como base
            if (!blast.nextRun) {
                blast.nextRun = new Date().toISOString();
                needsSave = true;
            }

            const nextRunDate = new Date(blast.nextRun);
            
            if (now >= nextRunDate) {
                console.log(`[SISTEMA] Executando Disparo em Grupo: ${blast.name}`);
                
                // Envia comando para o bot processar o envio stealth
                io.emit('bot:group-blast', {
                    sessionName: blast.targetBot,
                    groups: blast.groups,
                    message: blast.message,
                    stealthMode: blast.stealthMode,
                    campaignId: blast.id
                });

                // Calcula a próxima execução baseada no momento que deveria ter ocorrido (mais preciso)
                let newNextRun = new Date(nextRunDate);
                if (blast.frequency === 'hourly') {
                    newNextRun.setHours(newNextRun.getHours() + parseInt(blast.interval || 1));
                    // Se por algum motivo o servidor ficou offline e a data nova ainda é passado, joga pra frente
                    if (newNextRun <= now) newNextRun = new Date(now.getTime() + (parseInt(blast.interval || 1) * 3600000));
                } else {
                    // Diário: Próximo dia no mesmo horário
                    newNextRun.setDate(newNextRun.getDate() + 1);
                    if (blast.time) {
                        const [hrs, mins] = blast.time.split(':');
                        newNextRun.setHours(parseInt(hrs), parseInt(mins), 0, 0);
                    }
                }
                
                blast.nextRun = newNextRun.toISOString();
                blast.lastRun = now.toISOString();
                needsSave = true;
                
                // Atualiza a lista no frontend do dono
                setTimeout(() => {
                    const updatedCamps = JSON.parse(fs.readFileSync(CAMPAIGNS_FILE, 'utf8'));
                    io.to(blast.owner.toLowerCase()).emit('campaigns:list', updatedCamps.filter(c => c.owner === blast.owner));
                }, 1000);
            }
        }
        ...
        console.error("Erro no processador de grupos:", e);
    }
}, 30000); // Checa a cada 30 segundos para maior precisão
// Processador Automático da Fila de Cobranças (Roda a cada 1 minuto)
setInterval(async () => {
    const now = new Date();
    let queueUpdated = false;
    
    for (const owner in cobrancasQueue) {
        for (const item of cobrancasQueue[owner]) {
            if (item.status === 'pending' && new Date(item.scheduledDate) <= now && !item.processing) {
                item.processing = true;
                queueUpdated = true;
                
                let targetBotName = item.botName;
                if (targetBotName === 'manual') {
                    try {
                        const bots = await db.getAllBots();
                        const ownerBots = Object.values(bots).filter(b => b.owner === owner && b.status === 'Online');
                        if (ownerBots.length > 0) {
                            targetBotName = ownerBots[0].sessionName;
                        } else {
                            const anyOwnerBot = Object.values(bots).filter(b => b.owner === owner);
                            if (anyOwnerBot.length > 0) {
                                targetBotName = anyOwnerBot[0].sessionName;
                            }
                        }
                    } catch (e) {
                        console.error("Erro ao buscar bot para cliente manual:", e);
                    }
                }

                if (targetBotName === 'manual') {
                    item.status = 'error';
                    item.errorMsg = 'Nenhum robô disponível para enviar a cobrança.';
                    item.processing = false;
                    queueUpdated = true;
                    io.to(owner.toLowerCase()).emit('receive-fila-cobrancas', cobrancasQueue[owner]);
                    continue;
                }
                ...
                if (item.enviarPix) {
                    if (item.amount > 0) {
                        try {
                            ...
                            if (userMpToken) {
                                const description = `Renovação: ${item.providerName || 'Acesso'}`;
                                const external_reference = `mensal|${item.providerIndex}|${item.jid}|${owner}|${targetBotName}`;
                                
                                const reqMock = { headers: { host: 'localhost' }, body: { clientJid: item.jid, botSessionName: targetBotName }, connection: {} };
                                const result = await generatePix(reqMock, item.amount, description, external_reference, userMpToken);
                                
                                if (result && result.id) {
                                    pendingPayments[result.id] = {
                                        accessToken: userMpToken,
                                        paymentType: 'mensal',
                                        referenceId: item.providerIndex,
                                        clientJid: item.jid,
                                        botSessionName: targetBotName,
                                        createdAt: Date.now(),
                                        recoverySent: false
                                    };
                                    pixDataObj = {
                                ...
                                item.errorMsg = 'Token MP não configurado. PIX não gerado.';
                            }
                        } catch (e) {
                            console.error("[CRON COBRANCA PIX ERROR]", e.message);
                            item.errorMsg = 'Erro ao gerar PIX: ' + e.message;
                        }
                    } else {
                        item.errorMsg = 'Valor do plano inválido. PIX não gerado.';
                    }
                }
                
                // Dispara o comando de envio para o robô
                io.emit('bot:send-client-message', {
                    targetBot: targetBotName,
                    clientNumber: item.jid,
                    message: item.message,
                    pixData: pixDataObj,
                    messageId: item.id, // ID Único rastreável
                    clientId: item.jid,
                    owner: owner
                });
            }
    ...
    if (queueUpdated) {
        fs.writeFileSync(COBRANCAS_FILE, JSON.stringify(cobrancasQueue, null, 2));
    }
}, 60000);
...
const upload = multer({ 
    dest: 'uploads/', 
    limits: { fileSize: 10 * 1024 * 1024 }, // Aumentado para 20MB para suportar PDFs maiores
...
const loginLimiter = rateLimit({
    ...
    keyGenerator: (req) => {
        // Garante que o IP seja sempre uma string para evitar erro ERR_ERL_KEY_GEN_IPV6
        const clientIp = String(req.ip || req.headers['x-forwarded-for'] || req.socket.remoteAddress);
        return clientIp + (req.headers['user-agent'] || '').substring(0, 20);
    }
});
...
app.use(express.json({ limit: '20mb' })); // Limite de JSON aumentado para 20mb
app.use(express.urlencoded({ limit: '20mb', extended: true })); // Limite de Form aumentado para 20mb
...
// --- FUNÇÃO GLOBAL DE APROVAÇÃO DE PAGAMENTO ---
    async function processPaymentApproval(paymentData) {
        ...
        if (paymentType === 'campaign') {
        ...
        else if (paymentType === 'mensal') {
            // Lógica para Pagamento do Acesso Mensal do Cliente Final
            const pIndex = parseInt(referenceId);
            const clientJid = parts[2];
            const ownerUsername = parts[3];
            const botSessionName = parts[4];

            const safeOwner = String(ownerUsername).toLowerCase();
            let iptvObj = iptvData[safeOwner] || {};
            let provider = iptvObj.mensalProviders ? iptvObj.mensalProviders[pIndex] : null;

            let finalMessage = "✅ *Pagamento aprovado!* Seu acesso será liberado em breve.";
            ...
            if (provider) {
                if (provider.url) {
                    try {
                        ...
                        const phoneOnly = clientJid.replace('@s.whatsapp.net', '').replace(/[^0-9]/g, '');
                        const webhookFinalUrl = targetUrl.includes('?') 
                            ? `${targetUrl}&numero=${phoneOnly}&nome=Cliente` 
                            : `${targetUrl}?numero=${phoneOnly}&nome=Cliente`;

                        const axios = require('axios');
                        let response = null;
                        for (const method of ['GET', 'POST']) {
                            response = await axios({ method: method, url: webhookFinalUrl, timeout: 15000, validateStatus: () => true });
                            const responseDataStr = typeof response.data === 'string' ? response.data : JSON.stringify(response.data);
                            if (response.status === 405 || responseDataStr.toLowerCase().includes('method is not supported')) continue;
                            break;
                        }

                        let webhookReply = '';
                        if (typeof response.data === 'object') {
                            if (response.data.reply) webhookReply = response.data.reply;
                            else if (response.data.msg) webhookReply = response.data.msg;
                            else if (response.data.message) webhookReply = response.data.message;
                            else webhookReply = JSON.stringify(response.data, null, 2);
                        } else {
                            webhookReply = String(response.data).replace(/<br>/gi, '\n').replace(/<[^>]*>?/gm, '');
                        }
                        
                        try {
                            const regexExp = /(?:vencimento|validade|expira|v[áa]lido|at[ée])[^\n\d]*(\d{2})[\/\-](\d{2})[\/\-](\d{4})(?:[^\n\d]*(\d{2}):(\d{2})(?::(\d{2}))?)?/i;
                            let matchData = webhookReply.match(regexExp);
                            if (!matchData) matchData = webhookReply.match(/(\d{2})[\/\-](\d{2})[\/\-](\d{4})(?:[^\n\d]*(\d{2}):(\d{2})(?::(\d{2}))?)?/);
                        ...
                        let successMsg = iptvObj.mensalSuccessMsg ? iptvObj.mensalSuccessMsg : "✅ *Pagamento aprovado!* Aqui estão seus dados de acesso:";
                        finalMessage = successMsg + "\n\n" + webhookReply;
                    } catch(err) {
                        console.error("[MENSAL WEBHOOK ERRO]", err.message);
                        finalMessage = "✅ *Pagamento aprovado!* Houve uma lentidão ao gerar seu acesso automático. Nossa equipe já foi notificada e fará o envio por aqui.";
                    }
                } else {
                    let now = new Date();
                    now.setDate(now.getDate() + 30);
                    validadeISO = now.toISOString();
                    
                    let successMsg = iptvObj.mensalSuccessMsg ? iptvObj.mensalSuccessMsg : "✅ *Pagamento aprovado com sucesso!* Já estamos preparando o seu acesso e enviaremos por aqui em instantes.";
                    finalMessage = successMsg;
                }
            }

            // Envia a mensagem confirmando o pagamento e entregando o acesso
            io.emit('bot:send-client-message', {
                targetBot: botSessionName,
                clientNumber: clientJid.replace('@s.whatsapp.net', ''),
                message: finalMessage
            });

            // SALVA HISTÓRICO MENSAL NO GERENCIAMENTO
            let providerName = provider ? provider.name : 'Plano Mensal';
            if (!iptvData[safeOwner]) iptvData[safeOwner] = { enabled: false, history:[] };
            if (!iptvData[safeOwner].history) iptvData[safeOwner].history =[];

            let picUrl = null;
            let clientName = 'Cliente';
            if (leadsData[botSessionName] && leadsData[botSessionName][clientJid]) {
                picUrl = leadsData[botSessionName][clientJid].profilePicUrl;
                clientName = leadsData[botSessionName][clientJid].name || clientName;
            }

            iptvData[safeOwner].history.push({
                jid: clientJid,
                name: clientName,
                type: 'mensal',
            ...
            fs.writeFileSync(IPTV_FILE, JSON.stringify(iptvData, null, 2));
            
            // Atualiza a tela em tempo real no painel
            io.to(safeOwner).emit('receive-iptv-settings', iptvData[safeOwner]);
        }
...
async function generatePix(req, amount, description, external_reference, accessToken = null) {
    ...
    if (!tokenToUse || tokenToUse.includes('••••')) {
        throw new Error('Token do MercadoPago inválido ou não configurado.');
    }

    // Limpeza rigorosa do valor para evitar NaN
    const cleanAmount = parseFloat(String(amount).replace(',', '.'));
    if (isNaN(cleanAmount) || cleanAmount <= 0) {
        throw new Error('Valor do pagamento inválido.');
    }

    // --- IDENTIFICAÇÃO INTELIGENTE DO USUÁRIO ---
    let realEmail = `pagador_${Date.now().toString().slice(-6)}@zappbot.com.br`;
    ...
    if (req.session && req.session.user && req.session.user.username) {
        ...
        if (isEmail) {
            ...
            realEmail = `${safeName || 'user'}@zappbot.com.br`; 
        }
    ...
    if (!host || notificationUrl.includes('localhost') || notificationUrl.includes('127.0.0.1') || notificationUrl.includes('undefined')) {
    ...
    const body = {
        ...
        payer: { email: realEmail, first_name: firstName.substring(0, 20), last_name: "ZappBot" },
        external_reference: external_reference,
        metadata: {
            usuario_login: String(identificacaoReal).substring(0, 50),
            produto_escolhido: String(description).substring(0, 50),
            identificador_ref: String(external_reference).substring(0, 50),
            data_hora: dateStr
        }
    };
...
async function generatePreference(req, amount, description, external_reference, accessToken = null) {
    ...
    if (!tokenToUse || tokenToUse.includes('••••')) throw new Error('Token do MercadoPago inválido.');
    ...
    let notificationUrl = `${protocol}://${host}/webhook/mercadopago`;
    if (!host || notificationUrl.includes('localhost') || notificationUrl.includes('127.0.0.1') || notificationUrl.includes('undefined')) notificationUrl = null;
...
app.post('/api/create-payment', paymentLimiter, async (req, res) => {
    ...
    const users = await db.getAllUsers();
    const u = users[req.session.user.username.toLowerCase()];
    const parent = u && u.parentId ? users[u.parentId.toLowerCase()] : null;
    ...
    // LÓGICA INTELIGENTE: Se o usuário (ou Admin) tem créditos, avisamos o frontend para usar o saldo em vez de gerar PIX
    if (!planType.startsWith('resell_')) {
        let myCredits = u.isAdmin ? 999999 : (u.botLimit - 1);
        if (myCredits >= requiredCredits) {
            return res.json({ 
                useCredits: true, 
                creditsAvailable: myCredits, 
    ...
    if (planType && planType.startsWith('resell_')) {
        ...
        if (planType === 'resell_1') amount = parseFloat(pricesToUse.priceMonthly || settings.priceMonthly);
        ...
        // Nome amigável para pacotes de revenda e individuais
        desc = amountBots === 1 ? `1 Robô Adicional` : `Pacote Revenda: ${amountBots} Robôs`; 
    ...
    try {
        ...
        let resultPix = { point_of_interaction: { transaction_data: { qr_code: '', qr_code_base64: '' } } };
        let checkoutUrl = null;
        let paypalUrl = null;

        // Só gera Mercado Pago se o método for Pix/Cartão (Evita e-mail indesejado)
        if (method === 'mercadopago') {
            resultPix = await generatePix(req, amount, desc, extRef, tokenToUse);
            checkoutUrl = await generatePreference(req, amount, desc, extRef, tokenToUse);
        }
        
        // --- GERAÇÃO PAYPAL (MODO HIERÁRQUICO) ---
        let paypalClient = null;
        let paypalSecret = null;

        // Função auxiliar para limpar chaves de qualquer lixo ou máscara
        const cleanKey = (k) => (k && typeof k === 'string' && !k.includes('••')) ? k.trim() : null;

        // 1. Tenta as chaves do próprio usuário
        const uClient = cleanKey(u?.paypalClientId);
        const uSecret = cleanKey(u?.paypalClientSecret);

        if (uClient && uSecret) {
            paypalClient = uClient;
            paypalSecret = uSecret;
            console.log(`[DEBUG PAYPAL] Usando chaves do próprio usuário: ${u.username}`);
        } 
        // 2. Se não tiver, tenta as chaves do Revendedor (Pai)
        else {
            const pClient = cleanKey(parent?.paypalClientId);
            const pSecret = cleanKey(parent?.paypalClientSecret);
            
            if (pClient && pSecret) {
                paypalClient = pClient;
                paypalSecret = pSecret;
                console.log(`[DEBUG PAYPAL] Usando chaves do Revendedor: ${u.parentId}`);
            } 
            // 3. Fallback para as chaves do Admin (Global)
            else {
                paypalClient = cleanKey(settings.paypalClientId);
                paypalSecret = cleanKey(settings.paypalClientSecret);
                console.log(`[DEBUG PAYPAL] Usando chaves globais do Admin`);
            }
        }

        // LOG DE DIAGNÓSTICO REFORÇADO
        console.log("--------------------------------------------------");
        console.log("[SISTEMA] Tentando gerar PayPal...");
        console.log("[SISTEMA] ClientID:", paypalClient ? "OK (Inicia com " + paypalClient.substring(0,5) + "... Total: " + paypalClient.length + " chars)" : "AUSENTE");
        console.log("[SISTEMA] Secret:", paypalSecret ? "OK (Inicia com " + paypalSecret.substring(0,5) + "... Total: " + paypalSecret.length + " chars)" : "AUSENTE");
        
        if (paypalClient && paypalClient.startsWith('sb-')) {
            console.log("[AVISO] Chaves de SANDBOX detectadas. O sistema tentará o modo de teste.");
        }

        if (paypalClient && paypalSecret && paypalClient.length > 10) {
            try {
                paypalUrl = await generatePaypalOrder(amount, desc, extRef, paypalClient, paypalSecret, req.headers.host);
                console.log("[SISTEMA] Sucesso! Link PayPal gerado.");
            } catch (errP) { 
                console.error("❌ [ERRO API PAYPAL]:", errP.message);
                if (errP.response && errP.response.data) {
                    console.error("📦 [RESPOSTA DO PAYPAL]:", JSON.stringify(errP.response.data, null, 2));
                }
            }
        } else {
            console.log("[SISTEMA] PayPal ignorado: Chaves ausentes ou inválidas no banco de dados.");
        }
        console.log("--------------------------------------------------");
...
clientRoutes(io, generatePix, generatePaypalOrder, db, { startBotProcess, activeBots, updateBotStatus }, botEvents);
...
io.on('connection', async (socket) => {
    ...
    // Salva o histórico de IPTV gerado pelo Bot
    socket.on('bot:register-iptv-history', async (data) => {
        const bots = await db.getAllBots();
        const bot = bots[data.sessionName];
        if (bot && bot.owner) {
            const safeUser = String(bot.owner).toLowerCase();
            if (!iptvData[safeUser]) iptvData[safeUser] = { enabled: false, history: [] };
            if (!iptvData[safeUser].history) iptvData[safeUser].history =[];

            let picUrl = null;
            let finalName = data.name || 'Cliente';
            if (leadsData[data.sessionName] && leadsData[data.sessionName][data.jid]) {
                picUrl = leadsData[data.sessionName][data.jid].profilePicUrl;
                if (leadsData[data.sessionName][data.jid].name) {
                    finalName = leadsData[data.sessionName][data.jid].name;
                }
            }

            iptvData[safeUser].history.push({
                jid: data.jid,
                name: finalName,
                type: data.type, 
                ...
                picUrl: picUrl,
                botSessionName: data.sessionName
            });

            if (iptvData[safeUser].history.length > 500) {
                iptvData[safeUser].history = iptvData[safeUser].history.slice(-500);
            }

            fs.writeFileSync(IPTV_FILE, JSON.stringify(iptvData, null, 2));
            io.to(safeUser).emit('receive-iptv-settings', iptvData[safeUser]);
        }
    });
...
socket.on('livechat:subscribe-presence', async (data) => {
    ...
    socket.on('livechat:pause-ai', async (data) => {
        if (await checkBotOwnership(data.sessionName)) {
            const performer = user.username;
            io.to('bot_' + data.sessionName).emit('bot:pause-ai', { jid: data.jid, performer: performer });
            addUserLog(user.username, `Intervenção LiveChat no bot ${data.sessionName} para o cliente ${data.jid}`);
        }
    });
...
socket.on('livechat:resume-ai', async (data) => {
    ...
    socket.on('bot:message-status', (data) => {
        ...
        // --- ATUALIZA STATUS NA FILA DE COBRANÇAS (ERRO OU SUCESSO INICIAL) ---
        let queueUpdated = false;
        for (const owner in cobrancasQueue) {
            const item = cobrancasQueue[owner].find(i => i.id === data.messageId);
            if (item) {
                if (data.success) {
                    // REPETIR MENSAL: Atualiza a data do item atual para o próximo mês e volta para a fila
                    if (item.repetirMensal) {
                        const proximaData = new Date(item.scheduledDate);
                        proximaData.setMonth(proximaData.getMonth() + 1);
                        
                        item.scheduledDate = proximaData.toISOString();
                        item.date = proximaData.toISOString();
                        item.status = 'pending';
                        item.processing = false;
                        item.errorMsg = null;
                        item.realMsgId = null;
                        item.jaRepetiu = false;

                        // Atualiza a data de vencimento dentro do texto da mensagem (Padrão DD/MM/YYYY)
                        const match = item.message.match(/(\d{2})\/(\d{2})\/(\d{4})/);
                        if (match) {
                            let msgDate = new Date(`${match[3]}-${match[2]}-${match[1]}T12:00:00`);
                            msgDate.setMonth(msgDate.getMonth() + 1);
                            const newDateStr = `${String(msgDate.getDate()).padStart(2, '0')}/${String(msgDate.getMonth() + 1).padStart(2, '0')}/${msgDate.getFullYear()}`;
                            item.message = item.message.replace(/(\d{2})\/(\d{2})\/(\d{4})/g, newDateStr);
                        }
                    } else {
                        item.status = 'delivered'; // Fica como entregue até receber o 'read' (lido) do WhatsApp
                        item.realMsgId = data.realMsgId; // Salva o ID real do WA para rastrear os traços azuis
                    }
                } else {
                    item.status = 'error';
                    item.errorMsg = data.error || 'Erro desconhecido ao enviar (O número pode não ter WhatsApp).';
                }
                item.processing = false;
                queueUpdated = true;
        ...
        if (queueUpdated) fs.writeFileSync(COBRANCAS_FILE, JSON.stringify(cobrancasQueue, null, 2));
    });
...
socket.on('bot:message-status-update', async (data) => {
        ...
        const bot = bots[data.sessionName];
        if (bot && bot.owner) {
            const ownerKey = bot.owner.toLowerCase();
            // Repassa a informação de lido/entregue apenas para o dono do robô
            io.to(ownerKey).emit('livechat:message-status', data);
            
            // --- ATUALIZA STATUS EXATO NA FILA DE COBRANÇAS ---
            let queueUpdated = false;
            if (cobrancasQueue[ownerKey]) {
                const item = cobrancasQueue[ownerKey].find(i => i.realMsgId === data.msgId);
                if (item && (data.status === 'read' || data.status === 'delivered')) {
                    item.status = data.status; // Atualiza com a exatidão
                    queueUpdated = true;
                    io.to(ownerKey).emit('receive-fila-cobrancas', cobrancasQueue[ownerKey]);
                }
            }
            if (queueUpdated) fs.writeFileSync(COBRANCAS_FILE, JSON.stringify(cobrancasQueue, null, 2));
        }
    ...
    socket.on('get-public-prices', async (data) => {
            ...
            let prices = { 
                ...
                appUpdateMessage: s.appUpdateMessage, 
                ...
                hasPayPal: !!(s.paypalClientId && s.paypalClientSecret)
            };
        ...
        if (user) {
            ...
            if (u && u.botLimit > 1 && !u.isAdmin) {
                    ...
                    prices.hasPayPal = !!(u.paypalClientId && u.paypalClientSecret);
                }
            ...
            if (u && u.parentId && users[u.parentId]) {
                ...
                if (u.botLimit <= 1) {
                    if (parent.appName) prices.appName = parent.appName;
                    if (parent.supportNumber) prices.supportNumber = parent.supportNumber;
                    prices.hasPayPal = !!(parent.paypalClientId && parent.paypalClientSecret);
                }
            }
        }
    ...
    socket.on('update-group-settings', async (data) => {
        ...
        if (groups[data.groupId]) {
            ...
            const targetOwner = groups[data.groupId].owner;
            io.to(targetOwner.toLowerCase()).emit('group-list-updated', Object.values(updatedGroups).filter(g => g.owner === targetOwner));
            if (user.username.toLowerCase() !== targetOwner.toLowerCase()) {
                io.to(user.username.toLowerCase()).emit('group-list-updated', Object.values(updatedGroups).filter(g => g.owner === targetOwner));
            }
        ...
        setTimeout(async () => {
            ...
            if (groups[groupId]) { 
                if (groups[groupId].owner === ownerEmail) {
                    ...
                    groups[groupId].antiLink = false;
                    groups[groupId].prompt = '';
                    groups[groupId].silenceTime = 0;
                    groups[groupId].botName = '';
                    groups[groupId].isPaused = false;
                    groups[groupId].welcomeMessage = null;
                    groups[groupId].autoResponder = [];
                    groups[groupId].aiFallbackEnabled = true;
                    ...
                    io.to(ownerEmail.toLowerCase()).emit('feedback', { success: true, message: `Grupo "${groupName}" reativado e resetado!` });
                    
                    io.emit('group-activation-result', { success: true, groupId: groupId, botSessionName: botSessionName, expiresAt: groups[groupId].expiresAt, message: 'Grupo reativado e suas configurações foram resetadas!' });
                    return;
                } else {
            ...
            const now = new Date();
            let trialExpire = new Date(0); // 1970 (Já nasce expirado por padrão)
            const ownerData = users[ownerEmail];
            let status = "pending_payment";
            
            // Lógica Inteligente: Admin tem infinito, Usuário com crédito consome automático, Usuário novo ganha 24h.
            if (ownerData) {
                if (ownerData.isAdmin) {
                    trialExpire = new Date(now);
                    trialExpire.setFullYear(trialExpire.getFullYear() + 10); // Admin tem 10 anos
                    status = "active";
                } else if (ownerData.botLimit > 1) {
                    // Tem créditos! Consome 1 crédito automaticamente e dá 30 dias
                    ownerData.botLimit -= 1;
                    await db.saveUser(ownerData);
                    io.to(ownerEmail.toLowerCase()).emit('update-limit', ownerData.botLimit);
                    
                    trialExpire = new Date(now);
                    trialExpire.setDate(trialExpire.getDate() + 30);
                    status = "active";
            ...
            const newGroup = { groupId, groupName, owner: ownerEmail, managedByBot: botSessionName, status: status, antiLink: false, createdAt: now.toISOString(), expiresAt: trialExpire.toISOString(), prompt: "", silenceTime: 0, botName: "", isPaused: false };
    ...
    socket.on('client:request-mensal-pix', async (data) => {
        const { providerIndex, clientJid, botSessionName } = data;
        try {
            const bots = await db.getAllBots();
            const bot = bots[botSessionName];
            if (!bot) {
                io.emit('pix:generation-failed', { clientJid, botSessionName, message: 'Robô não encontrado no sistema.' });
                return;
            }
            const safeOwner = String(bot.owner).toLowerCase();
            const ownerData = (await db.getAllUsers())[safeOwner];
            const userMpToken = ownerData ? ownerData.mpAccessToken : null;

            if (!userMpToken) {
                io.emit('pix:generation-failed', { clientJid, botSessionName, message: 'O vendedor não configurou a conta do Mercado Pago para receber pagamentos.' });
                return;
            }

            const provider = iptvData[safeOwner].mensalProviders[providerIndex];
            if (!provider || !provider.price) {
                io.emit('pix:generation-failed', { clientJid, botSessionName, message: 'Pacote inválido ou sem preço configurado.' });
                return;
            }

            const amount = parseFloat(provider.price);
            if (isNaN(amount) || amount < 1) {
                io.emit('pix:generation-failed', { clientJid, botSessionName, message: 'O preço do pacote configurado é inválido (Mínimo R$ 1,00).' });
                return;
            }

            const description = `Acesso: ${provider.name}`;
            const external_reference = `mensal|${providerIndex}|${clientJid}|${safeOwner}|${botSessionName}`;
            
            const reqMock = { headers: socket.request.headers, body: { botSessionName, clientJid }, connection: socket.request.connection || {} };
            ...
            io.emit('pix:generated-for-client', { pixData, clientJid, botSessionName, isMensal: true, productName: provider.name });
        } catch (e) {
            console.error("Erro Mensal PIX:", e);
            io.emit('pix:generation-failed', { clientJid, botSessionName, message: 'Houve um erro no Mercado Pago ao gerar a cobrança.' });
        }
    });
    ...
    socket.on('client:update-pic', async (data) => {
        ...
        try {
            // Atualiza no clients.json (Gestor de Cobranças antigo)
            ...
            // Atualiza no iptv_data.json (Novo Gestor de Clientes/IPTV)
            const safeOwner = String(data.owner).toLowerCase();
            if (iptvData[safeOwner] && iptvData[safeOwner].history) {
                let updated = false;
                // Procura pelo JID (data.clientId geralmente é o JID sem o @s.whatsapp.net, então comparamos com includes)
                iptvData[safeOwner].history.forEach(item => {
                    if (item.jid && item.jid.includes(data.clientId) && item.picUrl !== data.picUrl) {
    ...
    if (user) {
        ...
        socket.emit('session-info', { 
            ...
            humanPauseTime: uData?.humanPauseTime || globalSettings.humanPauseTime || 2
        });
        ...
        socket.on('get-iptv-settings', async () => {
            const safeUser = String(user.username).toLowerCase();
            if (iptvData[safeUser]) {
                socket.emit('receive-iptv-settings', iptvData[safeUser]);
            } else {
                socket.emit('receive-iptv-settings', { enabled: false, webhookUrl: '', successMsg: '', mensalEnabled: false, mensalProviders:[], mensalSuccessMsg: '' });
            }
        });

        

        // --- INÍCIO: SOCKETS DA FILA DE COBRANÇAS ---
        socket.on('get-fila-cobrancas', () => {
            const safeUser = String(user.username).toLowerCase();
            socket.emit('receive-fila-cobrancas', cobrancasQueue[safeUser] ||[]);
        });

        socket.on('save-cobranca-automatica', (data) => {
            const safeUser = String(user.username).toLowerCase();
            if (!cobrancasQueue[safeUser]) cobrancasQueue[safeUser] = [];

            const history = iptvData[safeUser]?.history ||[];
            const providers = iptvData[safeUser]?.mensalProviders ||[];
            
            data.clientes.forEach(jid => {
                // Pega a validade mais recente desse cliente
                const clientHistory = history.filter(h => h.jid === jid && h.type === 'mensal').sort((a,b) => new Date(b.date) - new Date(a.date));
                if (clientHistory.length === 0) return;
                const clientData = clientHistory[0];

                let scheduledDate = new Date();
                if (data.regraTempo === 'custom') {
                    scheduledDate = new Date(data.dataEspecifica);
                } else {
                    if (clientData.validade) {
                        scheduledDate = new Date(clientData.validade);
                    }
                    if (isNaN(scheduledDate.getTime())) {
                        scheduledDate = new Date(); // Fallback para data de hoje se o cliente não tiver validade salva
                    }
                    const diasAntes = parseInt(data.regraTempo) || 0;
                    scheduledDate.setDate(scheduledDate.getDate() - diasAntes);
                    
                    let horas = 10;
                    let minutos = 0;
                    if (data.horarioEnvio) {
                        const parts = data.horarioEnvio.split(':');
                        if (parts.length === 2) {
                            horas = parseInt(parts[0]);
                            minutos = parseInt(parts[1]);
                        }
                    }
                    // Força o fuso horário para o horário escolhido no Brasil (UTC-3)
                    scheduledDate.setUTCHours(horas + 3, minutos, 0, 0); 
                }

                let finalMsg = data.mensagem
                    .replace(/{nome}/g, clientData.name)
                    .replace(/{vencimento}/g, clientData.validade ? new Date(clientData.validade).toLocaleDateString('pt-BR') : 'Sem data')
                    .replace(/{plano}/g, clientData.providerName || 'Plano');

                // Lógica para capturar o valor do plano para o PIX
                let amount = 0;
                let providerIndex = 0;
                if (data.enviarPix) {
                    let pIdx = providers.findIndex(p => p.name === clientData.providerName);
                    if (pIdx === -1 && providers.length > 0) pIdx = 0;
                    if (pIdx !== -1) {
                        amount = parseFloat(providers[pIdx].price) || 0;
                        providerIndex = pIdx;
                    }
                }

                cobrancasQueue[safeUser].push({
                    id: 'cob_' + Date.now() + '_' + Math.random().toString(36).substring(2,7),
                    jid: jid,
                    clientName: clientData.name || 'Cliente',
                    picUrl: clientData.picUrl,
                    botName: clientData.botSessionName,
                    scheduledDate: scheduledDate.toISOString(),
                    date: scheduledDate.toISOString(), // Usado para ordenação
                    message: finalMsg,
                    enviarPix: data.enviarPix,
                    amount: amount,
                    providerIndex: providerIndex,
                    providerName: clientData.providerName,
                    repetirMensal: data.repetirMensal,
                    status: 'pending',
                    processing: false,
                    errorMsg: null,
                    realMsgId: null,
        ...
        // --- FIM: SOCKETS DA FILA DE COBRANÇAS ---

        socket.on('edit-client', async (data) => {
            const safeUser = String(user.username).toLowerCase();
            if (!iptvData[safeUser]) return;
            
            let updated = false;
            // Busca o acesso mensal mais recente deste cliente para editar
            let clientHistory = iptvData[safeUser].history
                .filter(h => h.jid === data.jid && h.type === 'mensal')
                .sort((a,b) => new Date(b.date) - new Date(a.date));
            
            if (clientHistory.length > 0) {
                const latest = clientHistory[0];
                latest.validade = data.validade;
                
                if (data.providerName) {
                    latest.providerName = data.providerName;
                }
                updated = true;

                // Salva ou atualiza o preço no registro de provedores
                if (!iptvData[safeUser].mensalProviders) iptvData[safeUser].mensalProviders =[];
                
                let provider = iptvData[safeUser].mensalProviders.find(p => p.name === latest.providerName);
                if (provider) {
                    provider.price = data.price;
                } else {
                    iptvData[safeUser].mensalProviders.push({
            ...
            if (updated) {
                fs.writeFileSync(IPTV_FILE, JSON.stringify(iptvData, null, 2));
                socket.emit('receive-iptv-settings', iptvData[safeUser]);
                socket.emit('feedback', { success: true, message: 'Cliente atualizado com sucesso!' });
            } else {
                socket.emit('feedback', { success: false, message: 'Cliente não encontrado.' });
            }
        });

        socket.on('save-manual-client', async (data) => {
            const safeUser = String(user.username).toLowerCase();
            if (!iptvData[safeUser]) iptvData[safeUser] = { enabled: false, history: [] };
            if (!iptvData[safeUser].history) iptvData[safeUser].history =[];
            if (!iptvData[safeUser].mensalProviders) iptvData[safeUser].mensalProviders =[];

            // Adiciona o produto na lista de provedores mensais se não existir, para poder gerar PIX depois
            let providerExists = iptvData[safeUser].mensalProviders.find(p => p.name === data.providerName);
            if (!providerExists) {
                iptvData[safeUser].mensalProviders.push({
                ...
                // Atualiza o preço se já existir
                providerExists.price = data.price;
            }

            // Adiciona no histórico como 'mensal' para aparecer nas cobranças e na lista de clientes
            iptvData[safeUser].history.push({
                jid: data.jid,
                name: data.name,
                type: 'mensal',
                ...
                validade: data.validade,
                picUrl: null,
                botSessionName: 'manual' // Identificador de que foi adicionado manualmente
            });
            ...
            // Atualiza o frontend
            socket.emit('receive-iptv-settings', iptvData[safeUser]);
            socket.emit('feedback', { success: true, message: 'Cliente adicionado com sucesso!' });
        });

        socket.on('save-iptv-settings', async (data) => {
            const safeUser = String(user.username).toLowerCase();
            const existingHistory = iptvData[safeUser] && iptvData[safeUser].history ? iptvData[safeUser].history : [];
            ...
            iptvData[safeUser] = {
                ...
                mensalEnabled: data.mensalEnabled,
                mensalProviders: data.mensalProviders ||[],
                mensalSuccessMsg: data.mensalSuccessMsg,
                history: existingHistory // Mantém o histórico salvo!
            };
            fs.writeFileSync(IPTV_FILE, JSON.stringify(iptvData, null, 2));
            
            // Hot reload para os bots ativos desse usuário
            const bots = await db.getAllBots();
            Object.values(bots).forEach(bot => {
                if (bot.owner.toLowerCase() === safeUser && activeBots[bot.sessionName]) {
                    io.emit('bot-iptv-changed', {
                        sessionName: bot.sessionName,
                        iptvSettings: iptvData[safeUser]
                    });
                }
            });
        });
        
// --- NOVO: EXCLUIR REGISTROS DA FILA DE COBRANÇAS ---
        socket.on('delete-fila-cobrancas', async ({ itemsToDelete }) => {
            const safeUser = String(user.username).toLowerCase();
            if (cobrancasQueue[safeUser]) {
                cobrancasQueue[safeUser] = cobrancasQueue[safeUser].filter(
                    item => !itemsToDelete.includes(item.id)
                );
                fs.writeFileSync(COBRANCAS_FILE, JSON.stringify(cobrancasQueue, null, 2));
                socket.emit('receive-fila-cobrancas', cobrancasQueue[safeUser]);
                socket.emit('feedback', { success: true, message: `${itemsToDelete.length} registro(s) apagado(s) com sucesso!` });
            }
        });
// --- NOVO: EXCLUIR HISTÓRICO DE VENDAS/TESTES SELECIONADOS ---
        socket.on('delete-iptv-history', async ({ itemsToDelete }) => {
            const safeUser = String(user.username).toLowerCase();
            if (iptvData[safeUser] && iptvData[safeUser].history) {
                // Filtra o histórico mantendo apenas os que NÃO estão na lista de exclusão
                iptvData[safeUser].history = iptvData[safeUser].history.filter(
                    item => !itemsToDelete.includes(item.date)
                );
                // Salva no banco
                fs.writeFileSync(IPTV_FILE, JSON.stringify(iptvData, null, 2));
...
// --- NOVO: EXCLUIR CLIENTES SELECIONADOS ---
        socket.on('delete-iptv-clients', async ({ jidsToDelete }) => {
            const safeUser = String(user.username).toLowerCase();
            if (iptvData[safeUser] && iptvData[safeUser].history) {
                // Remove todos os registros 'mensal' que pertençam aos JIDs selecionados
                iptvData[safeUser].history = iptvData[safeUser].history.filter(
                    item => !(item.type === 'mensal' && jidsToDelete.includes(item.jid))
                );
                // Salva no banco
                fs.writeFileSync(IPTV_FILE, JSON.stringify(iptvData, null, 2));
                // Atualiza a tela na mesma hora
                socket.emit('receive-iptv-settings', iptvData[safeUser]);
                socket.emit('feedback', { success: true, message: `${jidsToDelete.length} cliente(s) apagado(s) com sucesso!` });
            }
        });
        socket.on('user:save-mp-token', async ({ token, language }) => {
            const users = await db.getAllUsers();
            if (users[user.username]) {
                users[user.username].mpAccessToken = token;
                if (language) users[user.username].language = language;
        ...
        socket.on('user:get-paypal-creds', async () => {
            const users = await db.getAllUsers();
            const u = users[user.username];
            if (u && (u.paypalClientId || u.paypalClientSecret)) {
                socket.emit('user:paypal-creds', { 
                    clientId: u.paypalClientId || '', 
                    clientSecret: u.paypalClientSecret || '' 
                });
            }
        });
        
        socket.on('user:save-paypal-creds', async (data) => {
            const users = await db.getAllUsers();
            if (users[user.username]) {
                users[user.username].paypalClientId = data.clientId;
                users[user.username].paypalClientSecret = data.secret;
                if (data.language) users[user.username].language = data.language;
                await db.saveUser(users[user.username]);
                socket.emit('feedback', { success: true, message: 'Credenciais PayPal salvas!' });
            }
        });
        ...
        if (user.isAdmin || (uData && uData.botLimit > 1)) {
            socket.on('admin-settings', async () => {
                ...
                const maskToken = (t) => t && t.length > 8 ? t.substring(0, 6) + '••••••••••' + t.substring(t.length - 4) : '';
                
                if (user.isAdmin) {
                    socket.emit('admin-settings', { 
                        ...s, 
                        mpAccessToken: maskToken(s.mpAccessToken),
                        paypalClientId: s.paypalClientId || '',
                        paypalClientSecret: maskToken(s.paypalClientSecret)
                    });
                } else {
                    const users = await db.getAllUsers();
                    const u = users[user.username];
                    const customSettings = { 
                        ...
                        humanPauseTime: u.humanPauseTime || s.humanPauseTime || 2,
                        paypalClientId: u.paypalClientId || '',
                        paypalClientSecret: maskToken(u.paypalClientSecret)
                    };
            ...
            socket.on('save-settings', async (ns) => { 
                const s = await db.getSettings();
                if (user.isAdmin) {
                    // Mantém os tokens antigos se o usuário salvou com asteriscos
                    ns.mpAccessToken = ns.mpAccessToken.includes('••••') ? s.mpAccessToken : ns.mpAccessToken;
                    ns.paypalClientSecret = (ns.paypalClientSecret && ns.paypalClientSecret.includes('••••')) ? s.paypalClientSecret : ns.paypalClientSecret;
                    ...
                    const newToken = ns.mpAccessToken.includes('••••') ? u.mpAccessToken : ns.mpAccessToken;
                    u.mpAccessToken = newToken;
                    
                    // --- SALVAMENTO PAYPAL REVENDEDOR ---
                    u.paypalClientId = ns.paypalClientId || '';
                    if (ns.paypalClientSecret && !ns.paypalClientSecret.includes('••••')) {
                        u.paypalClientSecret = ns.paypalClientSecret;
                    }
                    
                    u.appName = ns.appName;
                    u.supportNumber = ns.supportNumber;
                    u.humanPauseTime = ns.humanPauseTime || 2;
                    await db.saveUser(u);
                    
                    // Sincroniza o token para todas as partes da interface do usuário
                    socket.emit('user:mp-token', { token: u.mpAccessToken });
                    
                    // Sincroniza visualmente o token para o frontend
                    socket.emit('user:mp-token', { token: u.mpAccessToken });
            ...
            // INÍCIO: FUNÇÃO FALTANTE PARA SALVAR DIAS DO ROBÔ
            socket.on('admin-set-days', async ({ sessionName, days }) => {
                if (!user.isAdmin) return; // Apenas dono do sistema
                const bots = await db.getAllBots();
                const bot = bots[sessionName];
                if (bot) {
                    ...
                    await db.saveBot(bot);
                    io.emit('bot-updated', bot);
                    socket.emit('feedback', { success: true, message: 'Dias do robô atualizados com sucesso!' });
                }
            });
            // FIM: FUNÇÃO FALTANTE

            socket.on('admin-set-group-days', async ({ groupId, days }) => {
                if (!user.isAdmin) return; // Apenas dono do sistema pode forçar dias manuais
                const groups = await db.getAllGroups();
                ...
                if (group) {
                    ...
                    group.expiresAt = newDate.toISOString();
                    group.status = 'active';
                    await db.saveGroup(group);
                    
                    const updatedGroups = await db.getAllGroups();
                    const targetOwner = group.owner;
                    io.to(targetOwner.toLowerCase()).emit('group-list-updated', Object.values(updatedGroups).filter(g => g.owner === targetOwner));
                    if (user.username.toLowerCase() !== targetOwner.toLowerCase()) {
                        io.to(user.username.toLowerCase()).emit('group-list-updated', Object.values(updatedGroups).filter(g => g.owner === targetOwner));
                    }
                    socket.emit('feedback', { success: true, message: 'Dias do grupo atualizados com sucesso!' });
                }
            });
        ...
        // Retorna Leads aplicando a Hierarquia com PAGINAÇÃO para evitar travamentos
        socket.on('get-leads', async (params = {}) => {
            const users = await db.getAllUsers();
            const bots = await db.getAllBots();
            ...
            if (user.isAdmin) allowedUsers = Object.keys(users);
            else allowedUsers = allowedUsers.concat(getDescendants(myUsername));
            ...
            for (const sessionName of allowedBots) {
                if (leadsData[sessionName]) {
                    for (const number in leadsData[sessionName]) {
                        const lead = leadsData[sessionName][number];
                        const botInfo = bots[sessionName];
                        
                        // Filtro de busca no servidor (Otimiza memória do navegador)
                        if (search) {
                            const match = (lead.name || '').toLowerCase().includes(search) || 
                                          (number || '').toLowerCase().includes(search) ||
                                          (botInfo?.botName || sessionName).toLowerCase().includes(search);
                            if (!match) continue;
                        }

                        allLeads.push({
                            sessionName: sessionName,
                            botName: botInfo ? botInfo.botName || sessionName : sessionName,
                            number: number,
                            name: lead.name,
                            owner: lead.owner,
                            date: lead.date,
        ...
        socket.on('create-bot', async (d) => {
            ...
            try {
                ...
                // LÓGICA CORRIGIDA: Robô de Grupo atua apenas como Agregador. Ele não tem validade e não consome limite.
                // O limite/crédito será consumido apenas quando o usuário ativar os GRUPOS dentro dele.
                if (!ownerData.isAdmin && d.botType !== 'group') {
                ...
                if (ownerData.isAdmin || d.botType === 'group') {
                    ...
                    trialEndDate.setFullYear(trialEndDate.getFullYear() + 10); // 10 anos de validade (Infinito)
                    isTrial = false;
                    isActivated = true;
                    feedbackMessage = d.botType === 'group' ? 'Agregador de Grupos criado com sucesso!' : 'Criado (Admin).';
...
token: d.token,
        ...
        socket.on('pause-bot', async ({ sessionName }) => {
            ...
            if (activeBots[sessionName]) {
                ...
                io.to('bot_' + sessionName).emit('log-action', { msg: `⏸️ PAUSA MANUAL: Ativada pelo usuário [${performer}] via Painel.` });
                ...
                addUserLog(bot.owner, `Bot ${sessionName} pausado manualmente por ${performer}`);
            }
        });
        ...
        socket.on('resume-bot', async ({ sessionName }) => {
            ...
            if (activeBots[sessionName]) {
                ...
                io.to('bot_' + sessionName).emit('log-action', { msg: `🔊 RETOMADA MANUAL: IA religada pelo usuário [${performer}] via Painel.` });
                await updateBotStatus(sessionName, 'Online');
                socket.emit('feedback', { success: true, message: 'Bot retomado com sucesso!' });
                addUserLog(bot.owner, `Bot ${sessionName} retomado manualmente por ${performer}`);
            } else {
                if (bot) {
        ...
        socket.on('delete-bot', async ({ sessionName }) => {
            try {
                ...
                // ATENÇÃO: Robôs de Grupo não geram tempo de resgate, pois são agregadores gratuitos.
                const users = await db.getAllUsers();
                const ownerData = users[bot.owner];
                const now = new Date();
                const expire = new Date(bot.trialExpiresAt);
                
                if (ownerData && expire > now && bot.botType !== 'group') {
...
async function startBotProcess(bot, phoneNumber = null) {
    ...
    // Busca de forma segura no novo arquivo JSON
    const safeOwner = String(bot.owner).toLowerCase();
    const iptvSettingsObj = iptvData[safeOwner] || { enabled: false, webhookUrl: '', successMsg: '' };
    const iptvSettingsBase64 = Buffer.from(JSON.stringify(iptvSettingsObj)).toString('base64');
    ...
    p.on('close', async (code, signal) => { 
        ...
        if (isIntentional) {
           ...
           if (attempts.count <= 5) {
                ...
                const restartDelay = 10000 * Math.pow(2, attempts.count - 1);
...
// =================================================================================
// INTEGRAÇÃO PAYPAL SMART BUTTONS (NOVO MODELO SDK)
// =================================================================================
const pendingPaypalOrders = new Map();

async function getPaypalAccessToken(clientId, secret) {
    const baseUrl = clientId.startsWith('sb-') ? 'https://api-m.sandbox.paypal.com' : 'https://api-m.paypal.com';
...
// FUNÇÃO PARA GERAR O LINK DE PAGAMENTO DO PAYPAL
async function generatePaypalOrder(amount, description, external_reference, clientId, secret) {
    try {
        ...
        const order = await axios({
            ...
            data: {
                intent: 'CAPTURE',
                purchase_units: [{
                    reference_id: String(external_reference),
                    description: String(description),
                    amount: { currency_code: 'BRL', value: String(parseFloat(amount).toFixed(2)) }
                }]
            }
        });

        const approveLink = order.data.links.find(l => l.rel === 'approve').href;
        return approveLink;
    } catch (e) {
        console.error("[PAYPAL] Erro ao gerar link:", e.response?.data || e.message);
        return null;
    }
}

// Endpoint para o frontend saber qual ClientID carregar no SDK
app.get('/api/paypal/client-id', async (req, res) => {
    ...
    try {
        ...
        const settings = await db.getSettings();
        const u = users[req.session.user.username.toLowerCase()];
        const parent = u?.parentId ? users[u.parentId.toLowerCase()] : null;

        let clientId = u?.paypalClientId || parent?.paypalClientId || settings.paypalClientId;
        res.json({ clientId: clientId?.trim() || null });
...
app.post('/api/paypal/create-order', async (req, res) => {
    ...
    try {
        ...
        const clientId = (u?.paypalClientId && !u.paypalClientId.includes('••')) ? u.paypalClientId : (parent?.paypalClientId || settings.paypalClientId);
        const secret = (u?.paypalClientSecret && !u.paypalClientSecret.includes('••')) ? u.paypalClientSecret : (parent?.paypalClientSecret || settings.paypalClientSecret);

        if (!clientId || !secret) throw new Error("Credenciais PayPal não configuradas pelo vendedor.");
        ...
        pendingPaypalOrders.set(order.data.id, { external_reference, clientId, secret });
...
app.post('/api/paypal/capture-order', async (req, res) => {
    ...
    const orderData = pendingPaypalOrders.get(orderID);
    if (!orderData) return res.status(404).json({ error: "Ordem não encontrada ou expirada." });

    try {
        const { clientId, secret, external_reference } = orderData;
        const token = await getPaypalAccessToken(clientId, secret);
        const baseUrl = clientId.startsWith('sb-') ? 'https://api-m.sandbox.paypal.com' : 'https://api-m.paypal.com';
        ...
        if (capture.data.status === 'COMPLETED') {
            const detail = capture.data.purchase_units[0].payments.captures[0];
            await processPaymentApproval({
                status: 'approved',
                external_reference: external_reference,
                id: detail.id,
                transaction_amount: parseFloat(detail.amount.value)
            });
...
// Exporta funções necessárias para o client-routes.js evitando dependência circular
module.exports = { generatePaypalOrder };
