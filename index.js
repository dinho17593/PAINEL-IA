...
async function isBotAdminWA(sock, jid) {
    try {
        const me = sock.user || sock.authState.creds.me;
        if (!me) {
            console.log(`[${nomeSessao}] ❌ Erro isBotAdminWA: Dados do bot (me) não encontrados.`);
            return false;
        }
        ...
        const admins = await getGroupAdmins(sock, jid);
        console.log(`[${nomeSessao}] 🛡️ Verificando Admin: Bot JID: ${myJid} | Bot LID: ${myLid} | Admins no grupo: ${admins.length}`);
        ...
        console.log(`[${nomeSessao}] 🛡️ Resultado isBotAdminWA: ${isAdmin}`);
        return isAdmin;
    } catch (e) { 
        console.error(`[${nomeSessao}] ❌ Erro crítico em isBotAdminWA:`, e.message);
        return false; 
    }
}
...
if (platform === 'telegram') {
    ...
    async function ligarBot() {
        ...
        // Exporta para o escopo global para que os listeners externos (como o de buscar grupos) funcionem
        global.sock = sock;
...
sock.ev.on('messages.update', async (updates) => {
        ...
        sock.ev.on('messages.upsert', async ({ messages, type }) => {
            ...
            console.log(`[${nomeSessao}] 📥 Chegou algo no WhatsApp! Tipo: ${type}`);
            console.log(`[${nomeSessao}] 📥 Mensagem recebida no WhatsApp!`);
            console.log(`[${nomeSessao}] 📥 Nova mensagem detectada no WhatsApp!`);
            ...
            const isGroup = jid.endsWith('@g.us');
            console.log(`[${nomeSessao}] 🔍 Analisando Mensagem: Chat: ${jid} | É Grupo? ${isGroup} | BotType: ${botType}`);
            ...
            if (isGroup && botType !== 'group') {
                console.log(`[${nomeSessao}] 🛑 Bloqueio: Ignorando grupo porque o robô está configurado como INDIVIDUAL.`);
                return;
            }
            ...
            // --- VERIFICAÇÃO DE ATIVAÇÃO (AGORA NO TOPO) ---
            let tokenToActivate = null;
            if (texto.includes('/ativar?token=')) {
                tokenToActivate = texto.match(/token=([a-zA-Z0-9-]+)/)?.[1];
            } else if (texto.startsWith('/ativar') && texto.includes('token=')) {
            ...
            if (isHumanFromPhone && botType !== 'group') {
            ...
            if (pausados[jid] && Date.now() < pausados[jid]) {
                if (botType !== 'group' && (!msg.key.fromMe || isHumanFromPhone)) {
            ...
            if (botType === 'group') {
                if (!isGroup) return;
                if (!authorizedGroups[jid]) {
                    console.log(`[${nomeSessao}] ⚠️ Grupo ${jid} não está na lista de autorizados (Precisa ativar com o link).`);
                    return;
                }
                if (authorizedGroups[jid].expiresAt && new Date() > authorizedGroups[jid].expiresAt) {
                    console.log(`[${nomeSessao}] ❌ Grupo ${jid} está com a licença expirada.`);
                    return;
                }
                groupConfig = authorizedGroups[jid];
                if (groupConfig.isPaused) {
                    console.log(`[${nomeSessao}] ⏸️ Grupo ${jid} está pausado nas configurações.`);
                    return;
                }
            } else if (isGroup) {
                return;
            ...
            if (isGroup && botType === 'group') {
                ...
                if (texto.startsWith('!') || texto.startsWith('/') || texto.startsWith('.')) {
                    ...
                    console.log(`[${nomeSessao}] ⚙️ Comando detectado: ${comando}`);
                    
                    const senderIsAdm = await isGroupAdminWA(sock, jid, sender);
                    const botIsAdm = await isBotAdminWA(sock, jid);
                    
                    console.log(`[${nomeSessao}] 🛡️ Permissões -> Você é ADM: ${senderIsAdm} | Eu sou ADM: ${botIsAdm}`);
                    console.log(`[${nomeSessao}] 🛡️ Permissões: Você é ADM? ${senderIsAdm} | Eu sou ADM? ${botIsAdm}`);
                    ...
                    if (senderIsAdm) {
                        ...
                        switch (comando) {
                            ...
                            case 'banir':
                            case 'kick':
                                console.log(`[${nomeSessao}] 🔨 Comando BAN detectado. BotAdmin: ${botIsAdm} | Alvo: ${targetUser}`);
                                
                                if (!botIsAdm) {
                                    console.log(`[${nomeSessao}] ❌ Abortando BAN: Bot não é administrador do grupo.`);
                                    return sock.sendMessage(jid, { text: '❌ Preciso ser ADM para banir membros.' }, { quoted: msg });
                                }
                                
                                if (!targetUser) {
                                    console.log(`[${nomeSessao}] ❌ Abortando BAN: Nenhum usuário alvo identificado.`);
                                    return sock.sendMessage(jid, { text: '❌ Marque alguém, responda a uma mensagem ou digite o número para banir.' }, { quoted: msg });
                                }

                                try {
                                    console.log(`[${nomeSessao}] ⏳ Executando groupParticipantsUpdate (remove) para: ${targetUser}`);
                                    const response = await sock.groupParticipantsUpdate(jid, [targetUser], 'remove');
                                    console.log(`[${nomeSessao}] ✅ Resposta do WhatsApp para remoção:`, response);
                                    
                                    await sock.sendMessage(jid, { text: '✅ Usuário removido com sucesso!' });
                                } catch (errBan) {
                                    console.error(`[${nomeSessao}] ❌ Erro na execução do groupParticipantsUpdate:`, errBan.message);
                                    await sock.sendMessage(jid, { text: `❌ Erro ao tentar remover: ${errBan.message}` });
                                }
                                return;
